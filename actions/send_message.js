...
<br>
    ...
    <div style="width: calc(50% - 12px); float: right;">
      ...
      <span class="dbminputlabel">Ordem dos botões (1 - 25)</span>
  ...
  <div style="width: 100%; padding:8px;height: calc(100vh - 250px);overflow:auto">
      ...
      <dialog-list id="selectMenus" fields='["placeholder", "id", "tempVarName", "row", "min", "max", "mode", "time", "options", "actions", "disabled", "SelectMenuType"]' dialogTitle="Select Menu Info" dialogWidth="800" dialogHeight="700" listLabel="Menus" listStyle="height: calc(100vh - 310px);" itemName="Select Menu" itemCols="1" itemHeight="80px;" itemTextFunction="glob.formatItem3(data)" itemStyle="text-align: left; line-height: 40px;">
        <div style="padding: 16px;">


        <tab-system style="margin: 5px;">

            <tab label="Actions" icon="list">

            <action-list-input mode="SELECT" id="actions" height="calc(100vh - 200px)">
          ...
          <span class="dbminputlabel">Tipo de menu de Seleção</span>
          <select id="SelectMenuType" class="round">
            <option value="StringSelectMenu">Menu padrão</option>
  <option value="UserSelectMenu">Menu de membros do servidor</option>
  <option value="RoleSelectMenu">Menu de Cargos</option>
  <option value="MentionableSelectMenu">Menu de menção de membros</option>
  <option value="ChannelSelectMenu">Menu de canais</option>
  <option value="ChannelTextSelectMenu">Menu de canais de texto</option>
  <option value="ChannelVoiceSelectMenu">Menu de canais de voz</option>
  <option value="CategorySelectMenu">Menu de categorias</option>
  <option value="ChannelStageSelectMenu">Menu de canais de palco</option> 
  <option value="ChannelForumSelectMenu">Menu de foruns</option>
          </select>
        </div>
          ...
          <div style="width: calc(33% - 16px); float: left; margin-right: 16px;">
            ...
            <span class="dbminputlabel">Ordem dos menus (1 - 5)</span>
            <input id="row" placeholder="Deixe em branco para o padrão..." class="round" type="text">
...
.dbmmodsbr2{position:absolute;bottom:0px;border: 0px solid rgba(50,50,50,0.7);background:rgba(0,0,0,0.7);color:#999;padding:5px;right:0px;z-index:999999;cursor:pointer}
  ...
  async action(cache) {
    ...
    if (Array.isArray(data.buttons)) {
      for (let i = 0; i < data.buttons.length; i++) {
        ...
        if (result == true || fbot.formula == "3" || fbot.formula == "4" || fbot.formula == "5") {
          ...
          button.type = this.evalMessage(button.type, cache).replace("PRIMARY", 1);
          button.type = this.evalMessage(button.type, cache).replace("SECONDARY", 2);
          button.type = this.evalMessage(button.type, cache).replace("SUCCESS", 3);
          button.type = this.evalMessage(button.type, cache).replace("DANGER", 4);
          button.type = this.evalMessage(button.type, cache).replace("LINK", 5);
          ...
          if (button.typeper == "" || button.typeper == undefined) {
            ...
            if (check == "PRIMARY" || check == "SECONDARY" || check == "SUCCESS" || check == "DANGER" || check == "LINK" || check == "PREMIUM") {
          ...
          bt.setDisabled(data.buttons[i].disabled)
  
          if (button.emoji) bt.setEmoji(button.emoji);
  
          const urlValue = this.evalMessage(button.url, cache);
          if (button.type === "5") bt.setURL(urlValue);
          else bt.setCustomId(button.id);

   
                 prioridade = button.row || 1


          
          rowartificio.push(prioridade);
    ...
    if (Array.isArray(data.selectMenus)) {
      for (let i = 0; i < data.selectMenus.length; i++) {
        ...
        gatilhodesativar = select.disabled

        let desativarmenu
        if (gatilhodesativar == true) {
          desativarmenu = true;
        } else {
          desativarmenu = false;
        }
        ...
        let mn;
      menuType = select.SelectMenuType || "StringSelectMenu"
      ...
      if (menuType === "StringSelectMenu") {
        ...
        mn = new MentionableSelectMenuBuilder()
          .setCustomId(select.id)
          .setPlaceholder(select.placeholder)
          .setDisabled(desativarmenu)
          .setMinValues(parseInt(select.min) || 1)
        ...
        mn = new ChannelSelectMenuBuilder()
          .setCustomId(select.id)
          .setPlaceholder(select.placeholder)
          .setDisabled(desativarmenu)
          .setMinValues(parseInt(select.min) || 1)
          .setMaxValues(parseInt(select.max) || 1);
      } else if (menuType === "ChannelTextSelectMenu") {
        mn = new ChannelSelectMenuBuilder()
          .setCustomId(select.id)
          .setPlaceholder(select.placeholder)
          .setDisabled(desativarmenu)
          .setMinValues(parseInt(select.min) || 1)
          .setMaxValues(parseInt(select.max) || 1)
          .addChannelTypes(0); // Texto
      } else if (menuType === "ChannelVoiceSelectMenu") {
        mn = new ChannelSelectMenuBuilder()
          .setCustomId(select.id)
          .setPlaceholder(select.placeholder)
          .setDisabled(desativarmenu)
          .setMinValues(parseInt(select.min) || 1)
          .setMaxValues(parseInt(select.max) || 1)
          .addChannelTypes(2); // Voz
      } else if (menuType === "CategorySelectMenu") {
        mn = new ChannelSelectMenuBuilder()
          .setCustomId(select.id)
          .setPlaceholder(select.placeholder)
          .setDisabled(desativarmenu)
          .setMinValues(parseInt(select.min) || 1)
          .setMaxValues(parseInt(select.max) || 1)
          .addChannelTypes(4); // Categoria
      } else if (menuType === "ChannelStageSelectMenu") {
        mn = new ChannelSelectMenuBuilder()
          .setCustomId(select.id)
          .setPlaceholder(select.placeholder)
          .setDisabled(desativarmenu)
          .setMinValues(parseInt(select.min) || 1)
          .setMaxValues(parseInt(select.max) || 1)
          .addChannelTypes(13); // Palco (Stage Channel)
} else if (menuType === "ChannelForumSelectMenu") {
  mn = new ChannelSelectMenuBuilder()
    .setCustomId(select.id)
    .setPlaceholder(select.placeholder)
    .setDisabled(desativarmenu)
    .setMinValues(parseInt(select.min) || 1)
    .setMaxValues(parseInt(select.max) || 1)
    .addChannelTypes(15); // Fórum
}
    
       prioridade = select.row || 1

      if(prioridade > 1){
        prioridade = (prioridade * 5) - 4
      }

        rowartificio.push(prioridade);
        sendComponents.push(mn);
  ...
  let sortedIndices = rowartificio
    .map((row, index) => ({ row, index }))
    .sort((a, b) => a.row - b.row)
    .map((item) => item.index);

sendComponents = sortedIndices.map((index) => sendComponents[index]);
    ...
    sendComponents.forEach((comp) => {
      ...
      if (comp?.data?.type === 2) {
        if (ActionRow.components.length < 5) {
          ...
          ActionRow = new ActionRowBuilder().addComponents(comp);
        }
      } else {
        ...
        if (liberolum == true) {
          ...
          if (data.attachments[i].tipo == "1") {
            ...
            if (imagedata) {
              ...
              const msgAttachment = new AttachmentBuilder(buffer, name);
              if (spoiler) {

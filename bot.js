...
console.log(`Versão do Discord: \x1b[32m\x1b[1m${DiscordJS.version} ~ (Recomendada 14.18.0)\x1b[0m
Versão do NodeJS: \x1b[32m\x1b[1m${process.version} ~ (Recomendada 22.14.0)\x1b[0m`);
...
function PrintError(type) {
  ...
  switch (type) {
    case MsgType.MISSING_ACTION: {
      error(format("%s não existe!", arguments[1]));
      break;
    }
    case MsgType.DATA_PARSING_ERROR: {
      error(format("Houve um problema na análise %s!", arguments[1]));
      break;
    }
    case MsgType.MISSING_ACTIONS: {
      error(
        format(
          '[Actions ausentes]\nPor favor, copie a pasta "actions" do diretório do Discord Bot Maker para este diretório de bots: \n%s',
          arguments[1],
        ),
      );
      break;
    ...
    case MsgType.DUPLICATE_SLASH_COMMAND: {
      warn(
        format(
          '[Comando de barra duplicado]\nO comando Slash com o nome "%s" já existe!\nEsta duplicata será ignorada.\n',
          arguments[1]
        )
      );
      break;
    ...
    case MsgType.TOO_MANY_SPACES_SLASH_NAME: {
      warn(
        format(
          '[Muitos espaços no nome da barra]\nO comando de barra com o nome "%s" tem muitos espaços!\nOs nomes dos comandos de barra podem conter no máximo três palavras diferentes.\n',
          arguments[1]
        )
      );
      break;
    ...
    case MsgType.SUB_COMMAND_ALREADY_EXISTS: {
      warn(
        format(
          '[Subcomando já existe]\nO comando Slash com o nome "%s" não pode existir.\nEle requer a criação de um "grupo de subcomandos" chamado "%s",\nmas já existe um comando com esse nome.',
          arguments[1],
          arguments[2]
        )
      );
    ...
    case MsgType.SUB_COMMAND_GROUP_ALREADY_EXISTS: {
      warn(
        format(
          '[O grupo de subcomando já existe]\nO comando Slash com o nome "%s" não pode existir.\nJá existe um "subgrupo de comando" com esse nome.\nO "subgrupo de comando" existe por causa de um comando chamado algo como: "%s _____"',
          arguments[1],
          arguments[1]
        )
      );
    ...
    case MsgType.INVALID_SLASH_NAME: {
      error(
        format(
          '[Nome de comando de barra inválido]\nO comando Slash tem nome inválido: "%s".\nOs nomes dos comandos Slash não podem ter espaços e devem conter apenas letras, números, sublinhados e traços!\nEste comando será ignorado.',
          arguments[1]
        )
      );
      break;
    ...
    case MsgType.DUPLICATE_USER_COMMAND: {
      warn(
        format(
          '[Comando de usuário duplicado]\nO comando de usuário com o nome "%s" já existe!\nEsta duplicata será ignorada.\n',
          arguments[1]
        )
      );
      break;
    ...
    case MsgType.DUPLICATE_MESSAGE_COMMAND: {
      warn(
        format(
          '[Comando de mensagem duplicado]\nO comando de mensagem com o nome "%s" já existe!\nEsta duplicata será ignorada.\n',
          arguments[1]
        )
      );
      break;
    ...
    case MsgType.DUPLICATE_SLASH_PARAMETER: {
      warn(
        format(
          '[Comando de barra duplicado]\nO comando de barra "%s" parâmetro #%d ("%s") tem um nome que\njá está sendo usado!\nEsta duplicata será ignorada.\n',
          arguments[1],
          arguments[2],
          arguments[3]
        )
    ...
    case MsgType.INVALID_SLASH_PARAMETER_NAME: {
      error(
        format(
          '[Nome de parâmetro de barra inválido]\nO parâmetro #%d do comando de barra "%s" tem nome inválido: "%s".\nOs nomes dos parâmetros do comando de barra não podem ter espaços e devem conter apenas letras, números, sublinhados e traços!\nEste parâmetro será ignorado.\n',
          arguments[1],
          arguments[2],
          arguments[3]
        )
    ...
    case MsgType.INVALID_SLASH_COMMAND_SERVER_ID: {
      error(
        format(
          'ID de servidor inválido "%s" listado em "Opções de comando Slash -> IDs de servidor para comandos Slash"!\n'
        ),
        arguments[1]
      );
      break;
    ...
    case MsgType.DUPLICATE_BUTTON_ID: {
      warn(
        format(
          'A interação do botão com ID exclusivo "%s" já existe!\nEsta duplicata será ignorada.\n',
          arguments[1]
        )
      );
      break;
    ...
    case MsgType.DUPLICATE_SELECT_ID: {
      warn(
        format(
          'A interação do menu Selecionar com ID exclusivo "%s" já existe!\nEsta duplicata será ignorada.\n',
          arguments[1]
        )
      );
      break;
    ...
    case MsgType.MISSING_APPLICATION_COMMAND_ACCESS: {
      warn(
        format(
          'Os comandos Slash não podem ser fornecidos ao servidor: %s (ID: %s).\nConvide novamente o bot para este servidor usando o link de convite encontrado em "Configurações -> Configurações do bot".\nAlternativamente, você pode alternar para usar os Comandos Slash Globais em "Configurações -> Configurações do Comando Slash -> Preferência de Criação do Comando Slash". No entanto, observe que os comandos globais demoram muito para atualizar (~1 hora).',
          arguments[1],
          arguments[2]
        )
      );
    ...
    case MsgType.MISSING_MUSIC_MODULES: {
      warn(
        format(
          'Não foi possível carregar os módulos do Node relacionados a áudio.\nExecute "Arquivo -> Recursos de música -> Atualizar bibliotecas de música" para garantir que eles estejam instalados.'
        )
      );
      break;
    }
    ...
    case MsgType.MUTABLE_VOLUME_DISABLED: {
      warn(
        format(
          '[Volume mutável desabilitado]\nTentei definir o volume, mas "Volume mutável" está desabilitado.'
        )
      );
      break;
    }
    case MsgType.MUTABLE_VOLUME_NOT_IN_CHANNEL: {
      warn(
        format(
          "[Volume mutável não está no canal]\nTentei definir o volume, mas o bot não está em um canal de voz."
        )
      );
      break;
    }
    ...
    case MsgType.ERROR_GETTING_YT_INFO: {
      warn(format("Erro ao obter informações do YouTube.\n%s", arguments[1]));
    }

    case MsgType.ERROR_CREATING_AUDIO: {
      warn(format("Erro ao criar recurso de áudio.\n%s", arguments[1]));
    }

    case MsgType.MISSING_MEMBER_INTENT_FIND_USER_ID: {
      warn(
        "- Aviso do DBM - \nLocalizar usuário (por nome/ID) pode travar ou dar erro porque\no bot não habilitou a intenção de eventos de membros do servidor."
      );
    }
    case MsgType.CANNOT_FIND_USER_BY_ID: {
      warn(
        format(
          "[Não é possível encontrar o usuário pelo ID]\nNão é possível encontrar o usuário pelo ID: %s",
          arguments[1]
        )
      );
    }
    ...
    case MsgType.SERVER_MESSAGE_INTENT_REQUIRED: {
      warn(
        format(
          '[Intenção de conteúdo da mensagem necessária]\n%s comandos encontrados que exigem a intenção "Conteúdo da mensagem".\nEsses comandos exigem que o bot consiga ler mensagens dos servidores do Discord.\nPara habilitar esse comportamento, primeiro certifique-se de que a "INTENÇÃO DE CONTEÚDO DA MENSAGEM" esteja habilitada na seção "Bot" no Portal do desenvolvedor do Discord (a mesma página de onde você obteve seu token de bot).\nEm segundo lugar, no Discord Bot Maker, selecione Extensões -> Intenções de bot na barra de menu de título e, nesta caixa de diálogo, certifique-se de que "Conteúdo da mensagem" esteja marcado.',
          arguments[1]
        )
      );
      break;
    ...
    case MsgType.CHANNEL_PARTIAL_REQUIRED: {
      warn(
        format(
          '[Parcial do canal necessário]\nOs comandos %s estão definidos como "Somente DMs", mas o parcial do canal não está habilitado.\nPara permitir que o bot leia mensagens de DMs, faça o seguinte: No Discord Bot Maker, na barra de menu do título, vá para Extensões -> Parciais do bot.\nNa caixa de diálogo que aparece, selecione "Personalizado" e certifique-se de que "Canal (Habilita DMs)" esteja marcado.',
          arguments[1]
        )
      );
    }
